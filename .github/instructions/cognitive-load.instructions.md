...
```python
# High cognitive load
if (ticket.priority > 7 and ticket.channel == "email"
    and not ticket.is_triaged and minutes_since_creation > 30):
    escalate(ticket)

# Low cognitive load
is_high_priority = ticket.priority > 7
is_email_channel = ticket.channel == "email"
needs_triage = not ticket.is_triaged
is_overdue = minutes_since_creation > 30

if is_high_priority and is_email_channel and needs_triage and is_overdue:
    escalate(ticket)
```
...
def process(ticket: Ticket) -> Result:
