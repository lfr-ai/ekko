...
| Inappropriate Intimacy | Class accesses another's internals | Extract + Move |
...
| Duplicate Code | Identical blocks | Extract Method/Module |

## Commands

```bash
task test:unit                    # Verify after each step
uv run ruff check --fix .         # Fix lint issues
npx jscpd --reporters ai .       # Detect duplications
```
